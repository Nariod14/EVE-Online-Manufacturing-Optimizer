...
import { BlueprintTier, mockBlueprints } from "@/types/blueprints";
...
function MaterialsCell({ materials }: { materials: Material[] }) {
  return (
    <ul className="list-none pl-0 m-0 text-xs">
      {materials.map((mat, i) => {
        const category = Object.keys(mat)[0];
        const quantities = mat[category as keyof Material];
        return (
          <li key={i}>
            ...
            <ul className="list-none pl-3">
              {Object.entries(quantities).map(([item, qty]) => (
                <li key={item}>
                  <span className="text-blue-200">{item}</span>: {qty}
                </li>
              ))}
            </ul>
          </li>
...
export default function BlueprintsListTest({
    onEdit,
    onDelete,
    onMaxChange,
    onResetMax,
    ...
    onMaxChange?: (id: number, newMax: number | null) => void;
    onResetMax?: (id: number) => void;
    onResetAllMax?: (tier: "T1" | "T2") => void;
  }) {
...
function loadBlueprints() {
  ...
  setTimeout(() => {
    ...
    setLoading(false);
  }, 200); // 200ms delay for realism, optional
}
 ...
 // Allow any string for key, but only SortKey values are sortable
       ...
       <tr className="text-blue-400">
         {headers.map((h) => {
           ...
           return (
             <th
               key={h.key}
               className={`py-2 px-3 text-left ${isSortable ? "cursor-pointer select-none hover:underline" : ""}`}
               onClick={() =>
                 isSortable &&
                 setSort({
   ...
   return (
     <section className="w-full max-w-6xl mx-auto mt-8">
       <h2 className="text-2xl font-bold mb-6 text-blue-200">Current Blueprints</h2>
       <Accordion type="multiple" className="mb-4">
         {/* T1 Section */}
         <AccordionItem value="t1" defaultChecked>
           <AccordionTrigger
             className="text-lg font-semibold text-blue-100 bg-gradient-to-r from-blue-800 via-blue-700 to-blue-900 rounded-t-xl px-4 py-3"
             style={{
           ...
           <AccordionContent className="bg-gradient-to-br from-blue-950 via-blue-900 to-slate-950 rounded-b-xl px-2 pb-4 pt-2">
             { t1Blueprints.length ? (
               <div className="overflow-x-auto">
                 <table className="min-w-full text-sm bg-blue-950/70 rounded-xl shadow">
                   <thead>{renderHeader(t1Headers, t1Sort, setT1Sort)}</thead>
                   <tbody>
                     {sortBlueprints(t1Blueprints, t1Sort).map((bp) => {
                       ...
                       return (
                         <tr
                           key={bp.id}
                           className="border-b border-blue-800 hover:bg-blue-900/60 transition"
                         >
         ...
         <AccordionItem value="t2">
           <AccordionTrigger
             className="text-lg font-semibold bg-gradient-to-r from-orange-400 via-orange-500 to-yellow-500 text-orange-900 rounded-t-xl px-4 py-3"
             style={{
           ...
           >
             ...
             {t2Blueprints.length ? (
                 ...
                 <table className="min-w-full text-sm bg-[#2d1e13] rounded-xl shadow">
                     <thead>{renderHeader(t2Headers, t2Sort, setT2Sort)}</thead>
                     <tbody>
                       {sortBlueprints(t2Blueprints, t2Sort).map((bp) => {
                         if (bpTierCheck(bp)) {
                             ...
                             <tr
                                 key={bp.id}
                                 className="border-b border-[#ff9800] hover:bg-[#5a492f] transition"
                             >
                                 ...
                                 <td className="py-2 px-3 text-center text-[#ffedd1]">{profitPct}%</td>
